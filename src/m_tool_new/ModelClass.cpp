...
BOOL Model::MakeVertexLinks(VertexLink **VertexLinks)
{
	...
	// Initialize storage
	DWORD MemorySize = sizeof(VertexLink) * VerticesCount;
	...
	for (int i = 0; i < VerticesCount; i++)
	{
		...
		vl.Faces = new UINT [vl.FacesCount];
		vl.Indices = new UINT [vl.FacesCount];
		vl.FacesCount = 0;
	...
	if (StatusOk)
	{
		...
		for (int i = 0; i < FacesCount; i++)
		{
			...
			for (int j = 0; j < 3; j++)
			{
				...
				vl.Faces[vl.FacesCount] = (UINT) i;
				vl.Indices[vl.FacesCount] = (UINT) j;
...
BOOL Model::MakeVerticesNormalsEdged(VertexLink *VertexLinks, int Angle)
{
	...
	for (int i = 0; i < VerticesCount; i++)
	{
		...
		{
			UINT idx1 = vl1.Faces[j];
			UINT vert1 = vl1.Indices[j];
	...
	{
		// Copy unique normals
		DWORD MemorySize = sizeof(Vertex) * NewVerticesNormalsCount;
...
BOOL Model::MakeVerticesWeld(VertexLink *VertexLinks, int WeldDelta, BOOL IgnoreColors)
{
	...
	for (int i = 0; i < VerticesCount; i++)
	{
		...
		for (int j = i; j < VerticesCount; j++)
		{
			...
			if (Distance < WeldDelta)
			{
				...
				{
					// Indices of vertex (j)
					UINT idx2 = vl2.Faces[k];
					UINT vert2 = vl2.Indices[k];
	...
	if (StatusOk)
	{
		...
		DWORD MemorySize = sizeof(Vertex) * NewVerticesCount;
...
BOOL Model::MakeVerticesNormals()
{
	...
	UINT MemorySize = sizeof(Vertex) * NewVerticesNormalsCount;
...
BOOL Model::CalculateProjection(ProjectionParamsStruct &pps)
{
	...
	for (int k = 0; k < FacesCount; k++)
	{
		...
		if (abs(fn[2]) < FLT_EPSILON) continue;
...
BOOL Model::LoadBinaryFaces(HANDLE hFile)
{
	...
	for (int i = 0; i < FacesCount; i++)
	{
		...
		fv.Normals = new UINT [fsi.VerticesCount];
		fv.Vertices = new UINT [fsi.VerticesCount];
...
{
	f.Vertices[0] = (UINT) v1;
	f.Vertices[1] = (UINT) v2;
	f.Vertices[2] = (UINT) v3;
};
